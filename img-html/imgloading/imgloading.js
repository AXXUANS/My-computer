...
function imgloadimg(obj, parent, mode, wh) {
    ...
    var putObj = obj;
    if (obj.get(0).tagName.toLowerCase() == "img") {//获得节点并判断是否为img
        putObj = obj.parent(); //putobj为img的父级
        obj.hide();
    }
    obj.get(0).onload = null;//也有人注意到了要把img.onload 设置为null，只不过时机不对，大部分文章都是在callback运行以后，才将img.onload设置为null，这样虽然能解决循环引用的问题，
    var Container;
    if (parent == undefined) {
    ...
    imgload.onload = function () {
        ...
        switch (mode) {
            case 2:
                xy = zoom(this.width, this.height, cwidth, cheight);
                break;
            case 3:
                xy = zoomDoc(this.width, this.height, cwidth);
                break;
            default:
                xy = zoomscreent(this.width, this.height, cwidth, cheight);
                break;
        }
        img.width(xy.w);
        img.height(xy.h);
...
function zoomDoc(width, height, cwidth) {
    return { w: cwidth, h: parseInt(height * cwidth / width), t: 0, l: 0 };
}